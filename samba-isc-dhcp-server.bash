...
#Поскольку чаще всего по окончании работ все хранилища исполнителя подлежат форматированию (согласно ТЗ), возникла необходимость часто разворачивать файловый сервер на "чистой" операционной системе
#С целью экономии моего рабочего времени и был написан этот скрипт. Он предназначен для простой настройки файлового сервера Samba и DHCP-сервера isc-dhcp server (для экономии времени настройки клиентских устройств файлового сервера)
#Скрипт запрашивает данные для создания файлового и DHCP-сервера, а затем устанавливает службы в соответствии с параметрами, указанными пользователем
#Предполагается, что настройка сервера будет производиться в офисе с выходом в Интернет для дальнейшего отключения и перевозки на территорию заказчика
...
#Первым действием выводится запроос на ввод переменной server (IP-адреса настраиваемого сервра). 
#При вводе данных в переменную server осуществляется проверка - пользователю необходимо ввести переменную servervalidation, чтобы подтвердить значение ("$servervalidation" != "$server"). 
#Эта проверка нацелена на исключение шанса опечатки. Проверка на ввод пустого значения или значения или значения, отличного от IP-адреса, не добавлена т.к. предполагается, что работающий со скриптом сотрудник достаточно компетентен, чтобы не допускать подобных ошибок.
...
#Сперва определяется последний октет IP-адреса сервера для дальнейшей проверки переменных - выводится значение переменной $server, после чего посредством awk выделяется последний октет адреса и вносится в переменную $serverlsatoct
...
#Далее проверяется, не меньше ли указанный первый адрес пула, чем адрес сервера ("$firstaddrlastoct" -lt "$serverlastoct"). Если одно из условий не совпадает, начинается цикл while, в ходе которого производится повторный запрос на ввод переменной $firstaddr
...
#Следует отметить, что проверки строятся таким образом, что пул не может начинаться с адреса, меньше адреса сервера. Это теоретически может создать ситуацию, когда пул будет недостаточного размера (например, если адрес сервера равен 192.168.1.254)
...
#Перезапускается служба networking.service для обновления параметров сетевого интерфейса
#В Debian встречатся баг, из-за которого после однократного перезапуска сетевой службы адрес обновляется, однако вывод ifconfig является некорректным - проблема решается двойным перезапуском службы
...
#При использовании данного скрипта для настройки DHCP пул адресов всегда будет расчитываться из кол-ва адресов с маской подсети равной 24. Учитывая, что клиентами DHCP являются от пяти до десяти устройств, это было сочтено приемлемым
